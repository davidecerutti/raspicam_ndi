...
void setNDI(int *x_resolution, int *y_resolution, int *frame_rate, short int *format_type){		//set up parametres for the stream
	...
	bool fpchk = false, yreschk = false, xreschk = false, frchk = false;		//to check if the insert value is valide
	...
	switch (select){
		...
		//in the default request to inser manually all the information
			do{
				...
				cin>>*y_resolution;
				cout<<endl;
				if(*y_resolution <= 0){cout<<"error invalid value"<<endl; yreschk = false;}
				else{yreschk = true;}
			}while (yreschk == false);
			...
			do{
				...
				cin>>*x_resolution;
				cout<<endl;
				if(*x_resolution <= 0){cout<<"error invalid value"<<endl; xreschk = false;}
				else{xreschk = true;}
			}while (xreschk == false);
			...
			do{
				...
				cin>>*frame_rate;
				cout<<endl;
				if(*frame_rate <= 0){cout<<"error invalid value"<<endl; frchk = false;}
				else{frchk = true;}	
			}while (frchk == false);
			...
			do{
				...
				if(fpchk == false) {cout<<endl<<"error, invalid value"<<endl;}
			}while(fpchk ==  false);
	}
...
void setCamera( struct setting *setting_camera ){		//set up parametres for the camera
	...
	switch(selecter){
		case 1:
			...
			cout<<endl;
			if(setting_camera->brightness_val < 0){setting_camera->brightness_val = 0;}
			if(setting_camera->brightness_val > 100) {setting_camera->brightness_val = 100;}
			break;
		case 2:
			cout<<"inser a sharpness value between -100 and +100 	";
			cin>>setting_camera->sharpness_val;
			cout<<endl;
			if(setting_camera->sharpness_val < -100){setting_camera->sharpness_val = -100;}
			if(setting_camera->sharpness_val > 100){setting_camera->sharpness_val = 100;}
			break;
		case 3:
			...
			cout<<endl;
			if(setting_camera->contrast_val < -100){setting_camera->contrast_val = -100;}
			if(setting_camera->contrast_val > 100){setting_camera->contrast_val = 100;}
			break;
		case 4:
			...
			cout<<endl;
			if(setting_camera->saturation_val < -100){setting_camera->saturation_val = -100;}
			if(setting_camera->saturation_val > 100){setting_camera->saturation_val = 100;}
			break;
		case 5:
			...
			cout<<endl;
			if(setting_camera->shutter_speed_val <0){setting_camera->shutter_speed_val = 0;}
			if(setting_camera->shutter_speed_val > 330000){setting_camera->shutter_speed_val = 330000;}
			break;
		case 6:
			cout<<"inser a iso speed value between 100 and 800 		";
			cin>>setting_camera->ISO_val;
			cout<<endl;
			if(setting_camera->ISO_val < 100){setting_camera->ISO_val = 100;}
			if(setting_camera->ISO_val > 800){setting_camera->ISO_val = 800;}
			break;
		case 7:
			cout<<"activate camera stabilization? Y/N 		";
			cin>>stabint;
			if (stabint =='Y' || stabint =='y') {setting_camera->stabilization_val = true;}
			else if (stabint =='N' || stabint =='n') {setting_camera->stabilization_val = false;}
			else {cout<<"invalid value stabilization off"<<endl; setting_camera->stabilization_val = false;}
			cout<<endl;
			break;
		case 8:
			...
			cout<<endl;
			if(setting_camera->exposure_compensation_val < -10){setting_camera->exposure_compensation_val = -10;}
			if(setting_camera->exposure_compensation_val > 10){setting_camera->exposure_compensation_val = 10;}
			break;
		case 9:											//giving a number is more efficient than pass a Raspicam::object
			...
			cout<<endl;
			if(setting_camera->exposure_val < 0 || setting_camera->exposure_val > 12){setting_camera->exposure_val = 0;}
			break;
		case 10:										//giving a number is more efficient than pass a Raspicam::object
			...
			cout<<endl;
			if(setting_camera->awb_val < 0 || setting_camera->awb_val > 9){setting_camera->awb_val = 0;}
			break;
		case 11:
			cout<<"inser a white balance red component value between 0 and 8 		";
			cin>>setting_camera->setAWB_R;
			cout<<endl;
			if(setting_camera->setAWB_R < 0){setting_camera->setAWB_R = 0;}
			if(setting_camera->setAWB_R > 8){setting_camera->setAWB_R = 8;}
			break;
		case 12:
			cout<<"inser a green balance red component value between 0 and 8 		";
			cin>>setting_camera->setAWB_G;
			cout<<endl;
			if(setting_camera->setAWB_G < 0){setting_camera->setAWB_G = 0;}
			if(setting_camera->setAWB_G > 8){setting_camera->setAWB_G = 8;}
			break;
...
int main ( int argc,char **argv ) {		//main function
	...
	double Frame_rate_send;
			...
			std::cout<<"open camera setup?	Y/N		";
