...
app.post('', (req, res) => {
	...
	if (live_track == 0 && live_track != 1){
			...
			fs.writeFile("./CameraSettings.txt", FileStream , function(err) {
				if(err) { return console.error(err + "file not found"); }
			...
			fs.readFile('ndiconfigmodel.json', 'utf-8', (err, ndiconfigData)=>{
				...
				fs.writeFile('/home/pi/.newtek/ndi-config.v1.json', ndiconfigJSON , function(err) {
					if(err) { return console.error(err); }
		...
		NDISignal = exec('./Main');
		console.log("LOG: NDI stream started");
		...
		NDISignal.kill('SIGHUP');		
		console.log("LOG: NDI stream stopped");
...
app.listen(port, ()=> console.info('server is listening on port 8080'))
